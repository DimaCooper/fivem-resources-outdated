...
function ACItem (n = "Main menu", a = 0, t = "folder", e = "", c = "folder", p = false) {
	...
	this.select = function() {
		if (this.action == "repair") {
			...
			$.post("http://vrp_acustoms/setNUIFocus")
		} else if (type == "folder" && typeof(this.action) == "number") {
			setActiveMenu(menus[this.action]);
			$.post("http://vrp_acustoms/selectItem", JSON.stringify({
				num: this.action,
				mod: this.category,
				type: type
			}));
			...
			$.post("http://vrp_acustoms/addToCart", JSON.stringify({
				act: this.action,
				mod: this.category,
				par: par
			}));
	...
	this.preview = function() {
		...
		var menuPar = 0
		if (parent && parent.parent) menuPar = parent.parent.getIdx();
		$.post("http://vrp_acustoms/preview", JSON.stringify({
			act: this.action,
			mod: this.category,
			par: menuPar
		}));	
...
function docKeyPressed(cb) {
	if (cb.keyCode == 13) {
		$.post("http://vrp_acustoms/removeNUIFocus");
		...
		if (e.data.command == "createACMenu" && e.data.name) {
			...
			if (e.data.command == "nextACItem") {
				...
				if (activeMenu.parent) {
					...
					$.post("http://vrp_acustoms/goBack", JSON.stringify({
						num: i,
						command: "back",
					}));
				} else {

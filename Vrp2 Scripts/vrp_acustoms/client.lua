	...
	while (true) do
		...
		if inUse == false then
					...
					for i=0,#shops do
						...
						if GetDistanceBetweenCoords(sh.coords.x,sh.coords.y,0,GetEntityCoords(ped)) <= 8 then
							if IsControlJustPressed(1,201) then
								TriggerServerEvent("vrp_acustoms:checkCustPermisson")
								TriggerServerEvent("vrp_acustoms:setState", false)
								--TriggerServerEvent("vrp_acustoms:initVehPrice", string.lower(GetDisplayNameFromVehicleModel(GetEntityModel(veh))))
								vehPrice = 10000
								Citizen.Wait(500)
			...
			if confirmExit then
				...
				SetTextScale(1.0, 0.7)
				SetTextColour(255,255,255,200)
				AddTextComponentString(confirmExitText)

				DrawText(0.5, 0.85)				
			end
			if IsPauseMenuActive() then
				if not menuIsHidden then
					SendNUIMessage({command = "hideACMenu"})
					menuIsHidden = true
				end
			elseif menuIsHidden then
				SendNUIMessage({command = "showACMenu"})
				menuIsHidden = false
			end
...
function buildMenu(title)
	inUse = true
	SendNUIMessage({command = "openACMenu", name = title})
	...
	for i = 1,#menuInfo do
		...
		elseif iType == "rep" then
			SendNUIMessage({command = "addACItem", params = {par = menuInfo[i].parent, act = "repair", lText = fName, _class = "repair", rText = "", cat = "repair", prev = false}})
...
--[[
function getVehPrice(name)
	local types = garages.garage_types
	for i,tp in pairs(types) do
		for j,vh in pairs(tp) do
			if j == name then
				return tonumber(vh[2])
			end
		end
	end
	return defaultVehPrice
end
]]
...
RegisterNUICallback("install", function()
	TriggerServerEvent("vrp_acustoms:payGarage", totalPrice)
end)
...
RegisterNetEvent("vrp_acustoms:installMods")
RegisterNetEvent('vrp_acustoms:blockEnter')


AddEventHandler('vrp_acustoms:blockEnter', function (blockEnter)
...
AddEventHandler("vrp_acustoms:installMods", function()
	installMods()
	TriggerServerEvent("vrp_acustoms:setState", true)
	TriggerServerEvent("vrp_acustoms:saveState")

...
Citizen.CreateThread( function()
	...
	while true do
		...
		if IsControlJustPressed(1,207) and IsPedInAnyPoliceVehicle(GetPlayerPed(-1)) then
		...
		if radar.shown  then
			if radar.freeze == false then
					...
					if IsEntityAVehicle(e) then
						...
						radar.info = string.format("~y~Номер: ~w~%s  ~y~Модель: ~w~%s  ~y~Скорость: ~w~%s kph", fplate, fmodel, math.ceil(fvspeed))
					end
					...
					if IsEntityAVehicle(j) then
						...
						radar.info2 = string.format("~y~Номер: ~w~%s  ~y~Модель: ~w~%s  ~y~Скорость: ~w~%s kph", bplate, bmodel, math.ceil(bvspeed))

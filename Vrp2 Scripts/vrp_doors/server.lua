...
AddEventHandler('vrpdoorsystem:open', function(id)
  ...
  if user:hasPermission("!item."..cfg.list[id].key..".>0") or user:hasPermission(cfg.list[id].permission) then
      ...
      if cfg.list[id].locked then
        vRP.EXT.Base.remote.notify(user.source, "Дверь закрыта")
      else
        vRP.EXT.Base.remote.notify(user.source, "Дверь открыта")
      end
    end)
else
	vRP.EXT.Base.remote.notify(user.source, "У вас нет ключей от этой двери")
  end
end)

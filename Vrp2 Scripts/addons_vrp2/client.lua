...
function Addons:setPropSpike(prop)
    ...
    SetEntityHeading(object,heading+90)
end
...
function Addons:lockpickVehicle(wait,any)
		...
		if DoesEntityExist(vehicleHandle) then
		  if GetVehicleDoorsLockedForPlayer(vehicleHandle,PlayerId()) or any then
			...
			vRP.EXT.Base:notify(GetPlayerPed(-1),lang.lockpick.success) -- lang.lockpick.success()
			
		  else
			vRP.notify(GetPlayerPed(-1),lang.lockpick.unlocked) -- lang.lockpick.unlocked()
		  end
		else
			vRP.notify(GetPlayerPed(-1),lang.lockpick.toofar) -- lang.lockpick.toofar()
		end
end

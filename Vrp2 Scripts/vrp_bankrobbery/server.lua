  ...
  if user:hasPermission(cfg.cops) then
    ...
    if robbery then
	  if #cops >= robbery.cops then
		...
		vRP.EXT.Base.remote._notifyPicture(user.source, "CHAR_LESTER", 1, "ВНИМАНИЕ", "Тревога!", "Полиция получила вызов об ограблении!")
		  ...
		  SetTimeout(robbery.rob*1000, function()
			...
			if(robbers[savedSource])then
			  if user then
				...
				vRP.EXT.GUI.remote._announce(-1,"http://i.imgur.com/b2O9WMa.png", lang.robbery.complete({robbery.name}).. ""..reward.." $ было украдено!")
			  end
			end
		  end)
		end
